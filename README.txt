Naptha is used to dilute heavy crude oil (oil sands), though gas condensate and light crude oil can also be used

Graphics borrowed from BZGas

Respawn an ore patch when one is mined?

Drilling fluid, must drill oil well before can be pumped
Melting steel/Iron back into molten iron?
Direct Shipping Ore vs Benefication Ore

Recipe counts and throughput balancing
Ore grade mechanics (Fe% vs Impurities, Moisture)
Slag Chemistry system

Graphics borrowed from SpEx Bryle
In-Situ leaching?
Polymetallic nodules?


Minimum pre-industrial sources but few for certain trees like oil to bootstrap chemistry


Machines have wear and tear over time? https://mods.factorio.com/mod/MaintenanceMadness
Mines use explosives for blasting?
Blasting before surface mining? DEO document


Play around with Noise Expressions?
Remove entire patch on reservior running dry
Expoloration Well? Super fast drill that doesnt spawn well, but provides reservior info?
Corrosive resistant pipes?
Oil
-Drill requires drill pipe as an ingredient
-Sweet vs Sour fields?
-Methane Hydrates for ice planets?

-Blob patch (oil field) for bore drills? then spawn well patch under drill. Each oil field has associated drills which recieve output oil/second

Patch Types:
	Classic well
	Shale oil (Fracking)
	Oil Sands/Bitumen
	Oil Shale (Rock heated to make crude)
	Offshore
	Seeps/Tar Pits
	Condensate/NGL rich?


Gangue
Ores
- Iron
Ore Types:
    High-Grade Hematite (DSO)
    Magnetite (Less impurities, easier route to steel)
    Taconite (Hematite or Magnetite mostly, Banded Iron Formation)
    Titomagnetite
    Siderite (Carbonate ore and typically found with manganese, zinc or magnesium?)
	Nickel-Iron Alloy in space
- Copper
Notes:
	Mixed ore heavy (Oxides, Sulfides, Carbonates)
Ore Types:
	Porphyry (Copper Sulfide, Chalcocite)
	Copper Oxide deposit? (Malachite, Cuprite, Native Copper)
	Volcanogenic Vent (Copper/Zinc/Lead) (Chalcopyrite, Pyrite, Sphalerite)?
- Aluminum
Ore:
	Bauxite (Iron Oxide, Silica, titania impurities)
	Significant amount comes from recycling
	
	Alunite (hydrothermal/volcanic sulfate mineral studied for alumina production via roasting/leaching)
	Coal fly ash contains alumina
	Industrial/Ruined world allows for easy Aluminum recycling?
	Low/High grade bauxite?
- Uranium
Ore Types:
	Uranite
	Coffinite
	
	Mostly mined in situ
	Can come from Copper/Gold leaching solution?
	Lignite (Coal)
	Black Shale (Oil)
	Phosphorite (Flourine/Apatite)
- Lead
Ore Types:
	Polymetallic Ore (Lead/Zinc, also silver and less so copper/iron)

	Galena (Mineral found in combination with Zinc/Silver, ocassionally also iron/copper)
	Recovered from batteries
- Zinc
	See Above
- Chrome
- Cobalt
- Gold
- Magnesium
- Manganese
- Nickel
- Osmium
- Phosphorus
- Platinum
- Silicon
- Silver
- Tin
- Titanium
- Tungsten
- Thorium
- Flourite
- Coal
	Peat?
	Anthrocite
	Bitumen(ous?)
	Lignite

- Zircon?
- Strontium?
- Bismuth?
- Mercury?
- Beryllium?
- Rare Earth Oxides
- Molybdenum?
	-Porphry(Copper) is prodominent source
- Lithium
- Vandium
- Niobium
- Cadmium
- Tantalum
- Indium
- Gallium
- Rhenium